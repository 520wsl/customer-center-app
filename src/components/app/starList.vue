<template>
  <div class="star-list">
    <div v-for="(item,index1) in list" :key="index1" class="star-item" @click="changeChoose(index1)">
      ...
      <img v-if="item>=chooseNum && isHalf ==0" :src="$CDN('/star_default.png')">
      <!-- 半星情况 -->
      <img v-if="item<chooseNum && item!=chooseNum-0.5 && isHalf ==1" :src="$CDN('/star_choose.png')">
      <img v-if="item==chooseNum-0.5 && isHalf ==1" :src="$CDN('/star_choose.png')" class="clip-left">
      <img v-if="item==chooseNum-0.5 && isHalf ==1" :src="$CDN('/star_default.png')" class="clip-right">
      <img v-if="item>=chooseNum && item!=chooseNum-0.5 && isHalf ==1" :src="$CDN('/star_default.png')">
    </div>
  </div>
</template>
<script>
