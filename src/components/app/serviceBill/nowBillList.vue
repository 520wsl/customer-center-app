<template>
    <div class="now-bill-list" >
        <div v-infinite-scroll="loadMore" :infinite-scroll-disabled="loading" infinite-scroll-distance="10"
             v-if="billList.lenth != 0">
            <div class="now-bill-title" v-if="billList.length != 0"><img :src="$CDN('/service-sheet.png')">
                <span>在服工单</span>
            </div>
            <!--此处改为整个div跳转-->
            <router-link
                    :to="{ name: 'serviceBillInfo', query: { id: item.id,identity:2,companySixiId:companySixiId } }"
                    tag="div" class="case-item" v-for="(item,index) in billList" :key="index">
                <!-- <p class="BillId">
                    工单编号：
                ...
                <div class="case-item-header">
                    ...
                    <div class="info">
                        ...
                        <span class="evaluate-btn">
                            ...
                            <mt-button type="default" class="btn" v-if="!item.isCurrent"
                                       @click.stop="setCurrent(item, index)">设为当前工单</mt-button>
                            <mt-button type="default" class="btn" v-if="item.type < 2"
                                       @click.stop="revoke(item)">撤销</mt-button>
                            <mt-button type="default" class="btn" v-if="item.type == 2"
                                       @click.stop="finishWorkorder(item.id)">完结</mt-button>
                        </span>
                    </div>
                </div>
...
<script>
    import {
        getCompanyWorkSheetList,
        revokeWorkorder,
        setCurrentWorkorder,
        getCurrentWorkorderId,
        setWorkSheetState
    } from "@/api/workOrder/worksheet";
    ...
    export default {
        ...
        methods: {
            ...
            getWorkSheetList() {
                ...
                let customerSixiId = this.$store.state.User.wxUserInfo.customerSixiId || ""
                if (!customerSixiId) {
                    return
                }
                this.loading = true;
